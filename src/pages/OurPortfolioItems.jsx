...
const ArrowUpRightIcon = () => (
    // Simple underline for the link icon
    <span className="ml-1 underline">→</span>
);

// --- Mock Data for Portfolio (with more details) ---
 ...
 * Displays a single portfolio item with a vintage 90s look.
 */
const PortfolioCard = ({ item }) => {
    ...
    return (
        <motion.div
            className="bg-gray-200 border-2 border-t-white border-l-white border-r-gray-500 border-b-gray-500 p-2 flex flex-col"
        ...
        >
            <div className="relative h-56 border border-gray-500">
                <AnimatePresence>
                    <motion.img
                        ...
                        onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/600x400/cccccc/4a5568?text=Project+Image'; }}
                    />
                </AnimatePresence>
            </div>
            <div className="p-4 flex-grow flex flex-col bg-white mt-2 border border-gray-400">
                <h3 className="text-2xl font-bold text-black mb-1 font-serif">{item.title}</h3>
                <p className="text-sm text-gray-600 font-semibold mb-3">{item.category}</p>
                <p className="text-gray-800 text-sm mb-4 flex-grow font-serif">{item.description}</p>
                <div className="mb-4">
                    <p className="text-xs text-gray-500 font-medium mb-2">TECHNOLOGIES USED:</p>
                    <div className="flex flex-wrap gap-2">
                        {item.techStack.map(tech => (
                            <span key={tech} className="border border-gray-400 bg-gray-100 text-gray-700 text-xs font-mono px-2 py-0.5">{tech}</span>
                        ))}
                    </div>
                </div>
                <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-blue-700 hover:text-blue-800 underline text-sm font-semibold mt-auto inline-flex items-center group">
                    View Project
                    <ArrowUpRightIcon />
                </a>
            </div>
...
const Portfolio = () => {
    ...
    return (
        <div className="bg-gray-300 text-black min-h-screen p-4 sm:p-8 md:p-12 font-sans">
            <div className="max-w-7xl mx-auto">
                <div
                    className="text-center mb-4"
                >
                    <h1 className="text-4xl md:text-6xl font-bold mb-2 text-black font-serif">
                        Our Portfolio
                    </h1>
                    <p className="text-lg text-gray-800">Explore a curated selection of our finest work.</p>
                </div>
                <hr className="border-gray-500 border-b-2 mb-8" />
                ...
                <div className="flex justify-center flex-wrap gap-2 mb-12">
                    {categories.map(category => (
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-semibold border-2 transition-all duration-100 ${
                                activeFilter === category
                                    ? 'bg-gray-200 border-t-gray-600 border-l-gray-600 border-r-white border-b-white' // Inset effect
                                    : 'bg-gray-300 border-t-white border-l-white border-r-gray-600 border-b-gray-600 hover:bg-gray-200' // Outset effect
                            }`}
                        >
                            {category}
                        </button>
